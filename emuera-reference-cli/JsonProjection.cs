...
{
    internal static JsonNode? Value(AExpression expression) => expression.GetEraType() switch
    {
        EraType.Integer => JsonValue.Create(expression.GetIntValue(GlobalStatic.EMediator)),
        EraType.String => JsonValue.Create(expression.GetStrValue(GlobalStatic.EMediator)),
        EraType.Float => Floating(expression.GetFloatValue(GlobalStatic.EMediator)),
        _ => throw new InvalidOperationException("Expression has no runtime value"),
    };

    static JsonNode Floating(double value) => double.IsFinite(value)
        ? JsonValue.Create(value)!
        : JsonValue.Create(value.ToString("R", System.Globalization.CultureInfo.InvariantCulture))!;

    static string TypeName(EraType type) => type switch
    {
        EraType.Integer => typeof(long).FullName!,
        EraType.String => typeof(string).FullName!,
        EraType.Float => typeof(double).FullName!,
        _ => typeof(void).FullName!,
    };
    ...
    static JsonNode Word(Word word)
    {
        ...
        switch (word)
        {
            ...
            case LiteralFloatWord number: result["value"] = Floating(number.Float); break;
    ...
    internal static JsonNode Project(LabelDictionary labels)
    {
        ...
        foreach (var function in ordered)
        {
            ...
            functions.Add(new JsonObject
            {
                ...
                ["returnType"] = TypeName(function.MethodType),
