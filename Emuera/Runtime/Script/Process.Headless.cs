...
internal sealed partial class Process
{
	...
	internal void HeadlessSetRandomSeed(long seed)
	{
		...
		vEvaluator.HeadlessSetRandomSeed(seed);
	}

	internal void HeadlessLoadSave(string path)
	{
		if (!Program.HeadlessMode)
			throw new InvalidOperationException("Direct save loading is available only in headless mode");
		if (!vEvaluator.HeadlessLoadFrom(path))
			throw new CodeEE("Failed to load save data");
	...
	{
		if (!Program.HeadlessMode)
			throw new InvalidOperationException("Execution limits are available only in headless mode");
	...
	{
		if (!Program.HeadlessMode)
			throw new InvalidOperationException("Isolated calls are available only in headless mode");
	...
	{
		if (!Program.HeadlessMode)
			throw new InvalidOperationException("Statement execution is available only in headless mode");
		HeadlessCheckLimit();
		LogicalLine logicalLine = LogicalLineParser.ParseLine(source, console);
		if (logicalLine is not InstructionLine instruction)
